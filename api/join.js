...
export default async function handler(req, res) {
  // Extract code from the rewritten URL path e.g. /join/ROOMCODE
  ...
  if (code) {
    try {
      const r = await fetch(
        `${DB}/trips/${code}.json?shallow=false`,
...
<head>
  ...
  <!-- Universal link — iOS opens app directly if installed -->
  <meta name="apple-itunes-app" content="app-id=YOUR_APP_STORE_ID, app-argument=${appLink}" />
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-open:hover {
      ...
      box-shadow: 0 0 60px rgba(255,79,0,0.4), 0 8px 24px rgba(255,79,0,0.25);
    }
...
<div class="card">
  <div class="not-found">
    ...
    <a href="https://podplananything.com" class="btn-open" style="text-decoration:none;">
      ← Back to Pod
    </a>
  </div>
</div>
...
<div class="card">
  <div class="cover-wrap">
    ${pod.coverPhoto
      ? `<img class="cover" src="${pod.coverPhoto}" alt="${pod.name}" />
         <div class="cover-scrim"></div>`
      : `<div class="cover-fallback">
           <div class="cover-logo">
             <svg viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
               <circle cx="18" cy="18" r="10" fill="#FF4F00" opacity="0.9"/>
               <circle cx="18" cy="18" r="16" stroke="#FF4F00" stroke-width="1.5" opacity="0.3"/>
               <circle cx="18" cy="18" r="22" stroke="#FF4F00" stroke-width="1" opacity="0.12"/>
             </svg>
           </div>
         </div>`
    }
  ...
  <div class="body">
    ...
    <!-- Primary CTA — opens app if installed via universal link -->
    ...
    <a href="https://apps.apple.com/app/YOUR_APP_STORE_ID" class="btn-store">
...
<script>
  // If the deep link doesn't open the app within 1.5s, the fallback
  // App Store button is already visible — nothing extra needed.
  // This just makes the primary button feel snappier on iOS.
